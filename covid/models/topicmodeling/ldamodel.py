...
class LDAModel:
    ...
    def getLowerTopicBound(self, upper_bound):
        """
        ...
        max_num_clusters = upper_bound

        pca = TruncatedSVD(n_components=100)
        corpus_tfidf = tfidf[self.corpus]
        corpus_sparse = corpus2csc(
            corpus=corpus_tfidf, num_terms=num_terms, num_docs=len(corpus_tfidf)
        )
        ...
        bics = [
            GaussianMixture(n_components=num_comp, covariance_type="diag")
            .fit(corpus_pca)
            .bic(corpus_pca)
            for num_comp in tqdm(range(2, max_num_clusters))
        ]
        ...
        return (
            np.argmin(bics) + 2
        )  # counting at 2 + 1 for the index to get number of components
    ...
    def filter_low_tfidf(self, text_data, min_tfidf_score=0.01, keep_fraction=None):
        ...
        if keep_fraction:
            ...
            min_tfidf_score = np.percentile(
                list(id_to_scores.values()), (1 - keep_fraction) * 100
            )
        ...
        for bow in self.corpus:
            low_score_tokens += [
                ID for ID, score in tfidf[bow] if score < min_tfidf_score
            ]
    ...
    def filter_extremes(
        self, text_data, no_below=5, no_above=0.5, keep_n=100000, keep_tokens=None
    ):
        """
        ...
        self.id2word.filter_extremes(
            no_below=5, no_above=0.5, keep_n=100000, keep_tokens=None
        )
        self.corpus = self.create_corpus(text_data)
    ...
    def build_model(self, params, lda_class="single"):
        ...
        if params.get("per_word_topics"):
            raise ValueError(
                "Omit using parameter per_word_topics because it creates buggy models."
            )

        if lda_class == "single":
            self.lda_model = LdaModel(
                corpus=self.corpus, id2word=self.id2word, **params
            )
            ...
            os.environ.update(
                {"MALLET_HOME": MALLET_DIR}
            )  #'../covid/models/topicmodeling/mallet-2.0.8/'
            mallet_path = MALLET_DIR + "bin/mallet"
            mallet_model = LdaMallet(
                mallet_path, corpus=self.corpus, id2word=self.id2word, **params
            )
    ...
    def grid_search(
        self, text_data, param_grid, lda_class="single", scorers=["coherence_score"]
    ):
        """
        Input:
            - text_data: list of docs, i.e. list of lists of tokens
        ...
        for i, params in enumerate(tqdm(parameter_grid)):
            ...
            score = models[key][
                scorers[0]
            ]  # use first scorer in scorers to find the best model
            if best_score < score:
                best_score = score
                best_model_key = key
    ...
    def create_top_topic_per_doc_df(self, other_corpus=None):
        ...
        for doc in self.lda_model[corpus]:
            ...
            temp_df = pd.DataFrame(
                data=[dominant_topic, round(topic_prob, 4), topic_kws]
            ).T
    ...
    def getLDAVisualization(self):
        ...
        return pyLDAvis.gensim.prepare(
            self.lda_model, corpus=self.corpus, dictionary=self.id2word
        )
