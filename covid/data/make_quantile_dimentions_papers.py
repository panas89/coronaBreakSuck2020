...
def main(input_filepath, output_filepath, filename, classified_filepath, sheet_name, lb, ub):
    ...
    df_quantile = df.query('{low}<Altmetric<{high}'.format(low=low,high=high)).reset_index(drop=True)

    logger.info('Keeping only high impact papers ...')
    cols = ['Publication ID','Publisher','Source title','Altmetric']
    df_hi = df_classified.merge(df_quantile.loc[:,cols], how='inner', left_on='sha', right_on='Publication ID')
